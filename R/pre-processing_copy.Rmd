...
for (factor in 1:ncol(rawdata)) {
  if (length(table(rawdata[,factor])) < 10) {
    rawdata[,factor] <- as.factor(rawdata[,factor])
  }
}
...
### egfr data is still in both intiial and raw data frames ###
...
res <- knnImputation(df_init[-1], k = 10, meth = "median", scale = F) # do not use patient id in imputation
...
res <- cat_to_binary(res)
non_imputed <- cat_to_binary(non_imputed)
```

```{r echo = FALSE, results = 'asis'}

# add egfr information to non imputed data set
non_imputed$mu_egfr <- rawdata$mu_egfr
non_imputed$sigma_egfr <- rawdata$sigma_egfr
non_imputed$r_group <- rawdata$r_group
non_imputed$v_group <- rawdata$v_group

# remove all rows with NA
non_imputed <- non_imputed[complete.cases(non_imputed),]

# extract non_imputed_egfr data and remove for transformation
non_imputed_egfr_data <- non_imputed[68:71]
non_imputed <- non_imputed[-c(68:71)]
...
transform_continuous_data(non_imputed, "non imputed data")
...
res <- data.frame(res, rawdata$mu_egfr, rawdata$sigma_egfr, rawdata$r_group, rawdata$v_group)
...
incl_patients <- rawdata$patientid

t_untar <- as.data.frame(t(untar))

t_untar$vid <- vid
t_untar <- t_untar[t_untar$vid %in% incl_patients,]
